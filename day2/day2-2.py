...
int_codes_copy = int_codes.copy()


for noun in range(99):
    for verb in range(99):
        int_codes_copy = int_codes.copy()
        int_codes_copy[1] = noun
        int_codes_copy[2] = verb
        offset = 0

        while offset < len(int_codes)-3:
            code = int_codes_copy[0+offset]
            operand1 = int_codes_copy[1+offset]
            operand2 = int_codes_copy[2+offset]
            target = int_codes_copy[3+offset]

            if target < len(int_codes_copy):
                if code == 1:
                    int_codes_copy[target] = int_codes_copy[operand1] + int_codes_copy[operand2]
                elif code == 2:
                    int_codes_copy[target] = int_codes_copy[operand1] * int_codes_copy[operand2]
                elif code == 99:
                    break
